...
{
	SessionCache cache = Cache::loadSessionCache();
	if (cache.isValid) {
...
bool ParsecSession::deleteSessionCache() {
	bool success = Cache::deleteSessionCache();
	_isValid = false;
	Cache::cache.showParsecLogin = true;
	return success;
}
...
bool ParsecSession::saveSessionCache(bool isValid) {
	...
	bool success = Cache::saveSessionCache(cache);
...
const ParsecSession::SessionStatus ParsecSession::pollSession(ParsecSession::AuthResult auth)
{
	...
	if (responseSize > 0 && status == 201)
	{
		...
		if (success)
		{
			...
			this->type = SessionCache::SessionType::THIRD;
			_start = Clock::now();
...
const bool ParsecSession::fetchSession(const char* email, const char* password, const char* tfa)
{
	...
	if (responseSize > 0 && status == 201)
	{
		...
		this->type = SessionCache::SessionType::PERSONAL;
		_start = Clock::now();
		_expiry = 0;
		saveSessionCache();
